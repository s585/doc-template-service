...
import ru.sberbank.sbercrm.saas.doctemplate.template.converter.TemplateRecordConverter;
...
public class JooqTemplateRepository implements TemplateRepository {
    ...
    public List<Template> findAll(UUID tenantId, CommonRqDto request) {
        Condition condition = T_TEMPLATE.TENANT_ID.eq(tenantId)
            .and(jooqQueryBuilder.buildCondition(request.getFilter(), TemplateConstants.JooqFieldMappings.FIELDS));

        return dslContext.selectFrom(T_TEMPLATE)
            .where(condition)
            .orderBy(jooqQueryBuilder.buildOrderBy(request.getSort(), TemplateConstants.JooqFieldMappings.FIELDS))
