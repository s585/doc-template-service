...
- получение списка сгенерированных документов по `entityId + objectId` с
  пагинацией.

Список документов использует общий пагинируемый контракт сервиса.
Это пользовательская витрина готовых результатов:

- в список попадают только документы, у которых есть хотя бы один
  `generated_file` со статусом `DONE`;
- внутри каждого документа в ответе списка возвращаются только файлы со
  статусом `DONE`;
- `PENDING`, `PROCESSING` и `ERROR` файлы в списке игнорируются;
- статусный endpoint получения одного документа по `documentId` может
  возвращать файлы во всех статусах и используется для отслеживания процесса
  генерации.

План отдельного technical read API для истории generation зафиксирован в
`docs/generation-history-implementation-plan.md`.
