...
public class TemplateServiceImpl implements TemplateService {
    ...
    public PageResult<Template> findAll(UUID tenantId, CommonRqDto request) {
        List<Template> templates = templateRepository.findAll(tenantId, request);
        long totalRecordsAmount = templateRepository.count(tenantId, request);
        return PageResult.<Template>builder()
            .data(templates)
            .paging(CommonPageResponseBuilder.buildPaging(request, templates.size(), totalRecordsAmount))
            .build();
    }
